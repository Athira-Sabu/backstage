...
describe('NotificationsPanel', () => {
  ...
  it('should call delete API on delete', async () => {
    ...
    mockNotificationsApi.deleteNotifications.mockResolvedValue({ message: 'Notifications deleted successfully' });
    const component = await renderWrapped(<NotificationsPanel />);
    fireEvent.click(component.getByTestId('delete-btn'));

    expect(mockNotificationsApi.deleteNotifications).toHaveBeenCalledWith([1]);
    await waitFor(() => expect(component.queryByText('Notification 1')).not.toBeInTheDocument());
  });
  ...
  it('should call updateStatus API on update', async () => {
    ...
    mockNotificationsApi.updateStatus.mockResolvedValue({ message: 'Notification status updated successfully' });
    const component = await renderWrapped(<NotificationsPanel />);
